...
function kickPlayerMenu(player, playerSelected, lastMenu = 0) {
    if(!player.hasTag("op")) return;
    if(config.customcommands.kick.enabled === false) return player.tell("§r§6[§cPulse§6]§r Kicking players is disabled in config.js.");
    player.playSound("mob.chicken.plop");
    ...
    kickPlayerMenu.show(player).then((response) => {
        ...
        player.runCommandAsync(`tellraw @a[tag=op] {"rawtext":[{"text":"§r§6[§cPulse§6]§r "},{"text":"${player.nameTag} has kicked ${playerSelected.name} (Silent:${isSilent}). Reason: ${reason}"}]}`);
    });
}

function banPlayerMenu(player, playerSelected, lastMenu = 0) {
    if(!player.hasTag("op")) return;
    if(!config.customcommands.kick.enabled) return player.tell("§r§6[§cPulse§6]§r Banning players is disabled in config.js.");
    ...
    banPlayerMenu.show(player).then((response) => {
        ...
        player.runCommandAsync(`tellraw @a[tag=op] {"rawtext":[{"text":"§r§6[§cPulse§6]§r "},{"text":"${player.nameTag} has banned ${playerSelected.nameTag}. Reason: ${reason}"}]}`);
    });
}

function unbanPlayerMenu(player) {
    if(!player.hasTag("op")) return;
    if(!config.customcommands.unban.enabled) return player.tell("§r§6[§cPulse§6]§r Kicking players is disabled in config.js.");
    player.playSound("mob.chicken.plop");
    ...
    const unbanPlayerMenu = new MinecraftUI.ModalFormData()
        ...
        player.runCommandAsync(`tellraw @a[tag=op] {"rawtext":[{"text":"§r§6[§cPulse§6]§r "},{"text":"${player.nameTag} has added ${playerToUnban} into the unban queue. Reason: ${reason}"}]}`);
    });
}
...
export function playerSettingsMenuSelected(player, playerSelected) {
    ...
    if(!playerSelected.hasTag("op")) playerSettingsMenuSelected.button("Set Player as Pulse-Op", "textures/ui/op.png");
        else playerSettingsMenuSelected.button("Remove Player as Pulse-Op", "textures/ui/permissions_member_star.png");
    ...
    playerSettingsMenuSelected.show(player).then((response) => {
        if(response.selection === 0) {
            if(!config.customcommands.ecwipe.enabled) return player.tell("§r§6[§cPulse§6]§r Enderchest wiping is disabled in config.js.");
            let isOp;
            ...
            player.runCommandAsync(`tellraw @a[tag=op] {"rawtext":[{"text":"§r§6[§cPulse§6]§r "},{"text":"${playerSelected.name} has cleared ${player.name}'s enderchest."}]}`);
           ...
           kickPlayerMenu(player, playerSelected, 1);
            ...
            if(!config.customcommands.fly.enabled) return player.tell("§r§6[§cPulse§6]§r Toggling Fly is disabled in config.js.");
            if(playerSelected.hasTag("flying")) {
                playerSelected.runCommandAsync("function tools/fly");
                player.runCommandAsync(`tellraw @a[tag=op] {"rawtext":[{"text":"§r§6[§cPulse§6]§r "},{"text":"${player.name} has disabled fly mode for ${playerSelected.name}."}]}`);
                playerSettingsMenuSelected(player, playerSelected);
            } else {
                playerSelected.runCommandAsync("function tools/fly");
                player.runCommandAsync(`tellraw @a[tag=op] {"rawtext":[{"text":"§r§6[§cPulse§6]§r "},{"text":"${player.name} has enabled fly mode for ${playerSelected.name}."}]}`);
                playerSettingsMenuSelected(player, playerSelected);
            }
        } else if(response.selection === 4) {
            if(!config.customcommands.freeze.enabled) return player.tell("§r§6[§cPulse§6]§r Toggling Frozen State is disabled in config.js.");
            if(playerSelected.hasTag("frozen")) {
                playerSelected.runCommandAsync("function tools/freeze");
                player.runCommandAsync(`tellraw @a[tag=op] {"rawtext":[{"text":"§r§6[§cPulse§6]§r "},{"text":"${player.name} has unfrozen for ${playerSelected.name}."}]}`);
                playerSettingsMenuSelected(player, playerSelected);
            } else {
                playerSelected.runCommandAsync("function tools/freeze");
                player.runCommandAsync(`tellraw @a[tag=op] {"rawtext":[{"text":"§r§6[§cPulse§6]§r "},{"text":"${player.name} has frozen for ${playerSelected.name}."}]}`);
                playerSettingsMenuSelected(player, playerSelected);
            }
        } else if(response.selection === 5) {
            if(!config.customcommands.mute.enabled) return player.tell("§r§6[§cPulse§6]§r Muting players is disabled in config.js.");
            if(playerSelected.hasTag("isMuted")) {
                playerSelected.removeTag("isMuted");
                playerSelected.runCommandAsync("ability @s mute false");
                player.runCommandAsync(`tellraw @a[tag=op] {"rawtext":[{"text":"§r§6[§cPulse§6]§r "},{"text":"${playerSelected.name} has been unmuted by ${player.name}."}]}`);
                playerSettingsMenuSelected(player, playerSelected);
            } else {
                playerSelected.addTag("isMuted");
                playerSelected.runCommandAsync("ability @s mute true");
                player.runCommandAsync(`tellraw @a[tag=op] {"rawtext":[{"text":"§r§6[§cPulse§6]§r "},{"text":"${playerSelected.name} has been muted by ${player.name}."}]}`);
                playerSettingsMenuSelected(player, playerSelected);
            }
            ...
            if(playerSelected.hasTag("op")) {
                ...
                player.runCommandAsync(`tellraw @a[tag=op] {"rawtext":[{"text":"§r§6[§cPulse§6]§r "},{"text":"${playerSelected.name} is now Pulse-Opped by ${player.name}."}]}`);
                playerSettingsMenuSelected(player, playerSelected);
            }
        } else if(response.selection === 7) {
            if(!config.customcommands.vanish.enabled) return player.tell("§r§6[§cPulse§6]§r Toggling Vanish is disabled in config.js.");
            if(playerSelected.hasTag("vanished")) {
                playerSelected.runCommandAsync("function tools/vanish");
                player.runCommandAsync(`tellraw @a[tag=op] {"rawtext":[{"text":"§r§6[§cPulse§6]§r "},{"text":"${player.name} has put ${playerSelected.name} into vanish."}]}`);
                playerSettingsMenuSelected(player, playerSelected);
            } else {
                playerSelected.runCommandAsync("function tools/vanish");
                player.runCommandAsync(`tellraw @a[tag=op] {"rawtext":[{"text":"§r§6[§cPulse§6]§r "},{"text":"${player.name} has unvanished ${playerSelected.name}."}]}`);
                playerSettingsMenuSelected(player, playerSelected);
            }
