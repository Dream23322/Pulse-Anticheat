...
export function commandHandler(player, message) {
    ...
    if(commandData.requiredTags.length >= 1 && commandData.requiredTags.some(tag => player.hasTag(tag)) === false) {
        player.tell("§r§6[§cPulse§6]§r You need to be Pulse-Opped to use this command. To gain scythe-op run: /function op");
        return;
    }

    if(commandData.enabled === false) {
        player.tell("§r§6[§cPulse§6]§r This command has been disabled. Please contact your server admistrator for assistance.");
        return;
    }
    ...
    try {
        ...
        player.tell(`§r§6[§cPulse§6]§r There was an error while trying to run this command. Please forward this message to https://discord.gg/9m9TbgJ973.\n-------------------------\nCommand: ${String(message.message).replace(/"|\\/g, "")}\n${String(error).replace(/"|\\/g, "")}\n${error.stack || "\n"}-------------------------`);
    }
}
