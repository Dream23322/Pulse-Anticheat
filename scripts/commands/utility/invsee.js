...
export function invsee(message, args) {
    ...
    if(args.length === 0) return player.tell("§r§6[§cPulse§6]§r You need to provide whos inventory to view!");
    ...
    if(container.size === container.emptySlotsCount) {
        return player.tell(`§r§6[§cPulse§6]§r ${member.nameTag}'s inventory is empty.`);
    }

    let inventory = `§r§6[§cPulse§6]§r ${member.nameTag}'s inventory:\n\n`;
    
    for (let i = 0; i < container.size; i++) {
        const item = container.getItem(i);
        if(typeof item === "undefined") continue;

        inventory += `§r§6[§cPulse§6]§r Slot ${i}: ${item.typeId}:${item.data} x${item.amount}\n`;
    }
